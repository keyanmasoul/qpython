...
public class SL4AScriptService extends ForegroundService {
	protected final static int NOTIFICATION_ID = NotificationIdFactory.create();
	...
	public void onCreate() {
		super.onCreate();
		mInterpreterConfiguration = ((MNApp) getApplication()).getInterpreterConfiguration();
	}
	...
	public void onStart(Intent intent, final int startId) {
		...
		if (interpreter == null || !interpreter.isInstalled()) {
			...
			if (FeaturedInterpreters.isSupported(fileName)) {
				Intent i = new Intent(this, SL4ADialogActivity.class);
		...
		if (!script.exists()) {
			script = FileUtils.copyFromStream(fileName, getResources()
					.openRawResource(SL4AScript.ID));
		}
		copyResourcesToLocal(); // Copy all resources

		if (SL4AScript.getFileExtension(this)
